            ...
            if 'NaiveReplayMemory' in f:
                if 'durations' in f:
                    if 'mean' in f:
                        ...
                        label = f.replace('mean_','').replace('_durations.txt','').replace('_NaiveReplayMemory_prop_CartPole-v1','')
                        #label = f.replace('10000_', '').replace('mean_', '').replace('_durations.txt', '').replace('_MountainCar-v0','')
            ...
            if 'NaiveReplayMemory' in f:
                if 'rewards' in f:
                    if 'mean' in f:
                        ...
                        color, counter = get_color(counter)
                        label = f.replace('mean_','').replace('_rewards.txt','').replace('_NaiveReplayMemory_prop_CartPole-v1','')
                        #label = f.replace('10000_', '').replace('mean_', '').replace('_rewards.txt', '').replace('_MountainCar-v0', '')
                        if 'CombinedReplayMemory' in f or 'NaiveReplayMemory' in f:
                            label = label.replace('_prop','')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--file', default='_NaiveReplayMemory_prop_CartPole-v1_rewards', type=str,
                        help='name of file to read until numeric value') # 'NaiveReplayMemory_prop_durations', 
    parser.add_argument('--newFile', default='_NaiveReplayMemory_prop_CartPole-v1',type=str, help='Name of the file')
