...
def read_files(type):
    ...
    while exists:
        ...
        j += 1
        fd_name = ARGS.file+type+"%d.txt" % j
        print(fd_name)
        exists = os.path.isfile(fd_name)
...
def plot_comb_experiments():
    ...
    if not ARGS.common:
        ...
        episodes_data = read_files('_durations')
        print(episodes_data)
        ...
        rewards_data = read_files('_rewards')
            ...
            if '10000_' in f:
                if 'durations' in f:
                    if 'mean' in f:
                        ...
                        label = f.replace('10000_', '').replace('mean_', '').replace('_durations.txt', '').replace(
                            '_MountainCar-v0', '')
                        if 'CombinedReplayMemory' in f or 'NaiveReplayMemory' in f:
                            label = label.replace('_prop', '')
                        plot_data(data_mean1, std1, 'Episode durations per episode', color, label=label,
            ...
            if '10000_' in f:
                if 'rewards' in f:
                    if 'mean' in f:
                        ...
                        color, counter = get_color(counter)
                        label = f.replace('10000_', '').replace('mean_', '').replace('_rewards.txt', '').replace(
                            '_MountainCar-v0', '')
                        if 'CombinedReplayMemory' in f or 'NaiveReplayMemory' in f:
                            label = label.replace('_prop', '')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--file', default='10000_PER_rank_MountainCar-v0', type=str,
                        help='name of file to read until numeric value') # 'NaiveReplayMemory_prop_durations', 
    parser.add_argument('--newFile', default='10000_PER_rank_MountainCar-v0',type=str, help='Name of the file')
